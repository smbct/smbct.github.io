...
As I recently defended my thesis, and since I am now looking for a job, I started to dedicate to myself some time to study cutting edge computer science topics. Deep learning is one very famous (and somehow [controversial](https://www.youtube.com/watch?v=fe9u6YQTsWY)) topic I didn't have the occasion to experiment with. Thus I decided to implement an idea that I had during my thesis: inducing the rules of chess using a machine learning algorithm.
...
Since I needed a dataset for this project, I used this *csv* of [chess games](https://www.kaggle.com/datasets/datasnaek/chess), where the chess games are given in the form of lists of moves (easy to process with python-chess).

At this stage, I decided to simplify the learning task because predicting the possible (legal) moves from a board configuration implies using a model with a **varying output size**, something difficult to achieve with a deep neural net. I choose to **simplify** the problem by predicting only the piece moves, ignoring the interaction with the other pieces (which is quite cheating considering the initial idea, but interesting as a first step). I made a **second simplification** assuming that all the possible moves from the board position are known (which is not the case since only one possible move can be seen when someone plays).

> To sum up, if we take the illustration above, the main goal here is to produce a model able to predict a list of moves (the blue arrows) from a given board configuration (position of the pieces), assuming that from a given board configuration in the dataset, all the arrows are known.
{: .prompt-info }
...
For instance, coding the type of piece in the board with an integer won't be efficient since it would be too difficult for the model to decompose the integer.
...
For this reason, we are going to use convolutional layers for the model, a type of function that is efficient for treating spatial data such as images.
...
One popular way to encode categorical variables in deep learning is the one hot encoding, consisting in a vector of 0 and 1, with only one index equal to one depending on the class that is coded.
Our encoding here will be based on same principle, inspired from encoding used for existing [chess AI](https://erikbern.com/2014/11/29/deep-learning-for-chess.html) (including the alphazero model).
The categorical will represent a spatial dimension of the board in order to help the model consider several piece types at the same time.
Thus, the final board encoding will be represented as a 3d board, where the depth dimension (depth = 2*6) represents the type of piece.
By doing so, the model will be able to perform computations involving different types of pieces at the same time.
You can visualize the input size on the figure below, where the first board layers have been shown for the white pieces.
...
Additionally to the board, we will also represent as input the move to perform.
This will be encoded as a two dimensional 0/1 array 8x8 array, with the value one corresponding to the piece to play.
...
Our model output consists of all the possible moves of the piece indicated in the input.
Since we are not considering encoding information such as piece capture, the output will simply consider of a 0/1 8x8 array with a 1 value for each possible destination of the piece.
...
Once again, I took inspiration from existing models such as deepmind's alphazero for this design.
...
This way, all the intermediate inputs are 8*8 arrays, and I also introduced residual connections between the layers.
...
Initially, the dataset was composed of only the boards from the first games of the *csv* files, but I realized that it was not diverse enough since the board configuration does not evolve much from few plays.
...
You will find below several board comparisons from one particular game.
...
We can also see that illegal moves are not well managed by the model.
...
It will be for instance interesting to find a way to encode the capture and to learning only from the game configurations (instead of using directly the rules).
...
Looking at the efficiency of neural nets makes me think we are far from understanding the possible connections between continuous and logical computation.
...
So we may still think that from its playing, this type of model is somehow able to understand the game, but whether it may output the rules remains unclear.
I also find this [discussion](https://www.reddit.com/r/chess/comments/p7gpgg/can_a_modern_machine_learning_algorithm_learn_the/) on the topic, and that [one](https://softwareengineering.stackexchange.com/questions/92189/learning-the-rules-of-chess).
...
Please feel free to re-use and improve the model (and let me know if you are able to improve the result) !
