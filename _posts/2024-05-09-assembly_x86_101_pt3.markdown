...
Our goal in this post we will to start from our square drawing program and modify it in order to draw a circle.
More importantly, we will see how to properly store our variables instead of using registers. 
...
_start:
    ...
    ; allocating 4 bytes in the stack
    mov rbp, rsp ; save the current base address of the stack
...
By interacting with the stack to store our variables, our program is now muche cleaner than before.
There is still a lot to say about the stack, in particular how to properly manage it for function calls.
We will discuss about it in the next part of the series.
...
Once agin, many improvements are possible such as defining a radius for the circle that is different from the square size.
